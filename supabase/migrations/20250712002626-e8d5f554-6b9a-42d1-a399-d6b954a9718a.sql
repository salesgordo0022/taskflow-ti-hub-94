...
-- Estrutura completa das tabelas para o sistema de gestão
...
CREATE POLICY "Authenticated users can access system_companies" ON public.system_companies
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can access system_tags" ON public.system_tags
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can access system_users" ON public.system_users
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can access tasks" ON public.tasks
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can access task_reminder_channels" ON public.task_reminder_channels
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can access subtasks" ON public.subtasks
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can access incidents" ON public.incidents
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can access incident_systems" ON public.incident_systems
  FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can access incident_notes" ON public.incident_notes
  FOR ALL USING (auth.role() = 'authenticated');
...
('Empresa Exemplo LTDA', '12.345.678/0001-90', 'João Silva', 'João Silva', 'contato@exemplo.com', '(11) 99999-9999', 'comercio', 'simples', 'medio'),
('Tech Solutions S.A.', '98.765.432/0001-10', 'Maria Santos', 'Maria Santos', 'info@techsolutions.com', '(11) 88888-8888', 'servicos', 'presumido', 'dificil');

INSERT INTO public.systems (name, version, description, responsible, status, start_date, expected_end_date, progress) VALUES
('Sistema ERP', '1.0.0', 'Sistema integrado de gestão empresarial', 'Equipe TI', 'in_progress', NOW(), NOW() + INTERVAL '6 months', 75),
